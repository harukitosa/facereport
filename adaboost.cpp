...
void adaboost(cv::Mat* images, int* labels, int sample_num)
{
  ...
  for(int t = 0; t < T; t++){   
    ...
    if(min_error[t] == 0){
      ...
      for (int k = 0;k < sample_num;k++) {
        if (min_error[t] != 0) {
          w[k] = w[k]*std::pow(min_error[t]/(1.0-min_error[t]), 1.0-min_error[t]);
          if (k == 0){
            std::cout << w[k]*std::pow(min_error[t]/(1.0-min_error[t]), 1.0-min_error[t]) << std::endl;
            std::cout << "min_error[" << k << "]" << min_error[t] << std::endl;
  ...
  for(int i = 0; i < T; i++){
    ofs << f_hx[T_idx[i]] << ", " << f_hy[T_idx[i]] << ", " << f_h[T_idx[i]] 
	<< ", " << f_w[T_idx[i]] << ", " << f_flag[T_idx[i]] << ", " << min_thrd[i] 
	<< ", " << min_error[i] / (1 - min_error[i]) << ", " <<  min_p[i] << std::endl;
  }
}
